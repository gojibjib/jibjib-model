  ...
  for dirName, subdirList, fileList  in os.walk(rootDir):
    ...
    for fname in fileList:
      if fname.endswith(".wav"):
          ...
          encoded[counter-1]=1
          #encoded = tf.one_hot(counter,_NUM_CLASSES)
          #print(encoded)
  ...
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    for step in range(FLAGS.num_batches):      
      ...
      for i in range(0, len(X_train), minibatch_size):
        ...
        [num_steps, loss,_, train_acc,temp] = sess.run([global_step_tensor, loss_tensor, train_op,accuracy,prediction],feed_dict={features_tensor: X_train_mini, labels_tensor: y_train_mini})
        ...
        if (num_steps)%1 == 0:
          ...
          val_acc, = sess.run([accuracy], feed_dict={features_tensor: X_test, labels_tensor: y_test})
